...
body<-dashboardBody(tags$head(HTML("
  ...
  shinybusy::busy_start_up(
    ...
    text = "Chargement...",
    mode = "timeout",background = "#00735B",timeout = 2000,color = "white"
  ),
               ...
               tags$div(class="green_div",
                        tags$div(id="accueil_text",
                                 ...
                                 h3("+ de 1000 Bonnes Pratiques"),tags$div(id="collectivite?",
                                                                           pickerInput(inputId="avec_collectivite",label = NULL,choices=per$INSEE,multiple=T,options = pickerOptions(liveSearch = T,size = 5,`none-selected-text` = "Saisir une collectivité")),
...
server <- function(input, output, session) {
  ...
  # 
  #   if (OK[1,1]==1) {
  #     updateTabItems(inputId = "sidebarID",selected = "accueil")
  #     print("ok")
  #   } else {
  #     
  #   }
...
print(filtre_compa)
  ...
  observeEvent(c(input$Motscles,input$Habitants,input$Thematiques,input$Pays,input$avec_collectivite),{
      ...
      if (!is.null(input$avec_collectivite)) {
        ...
        for (i in 1:length(BDD$ANNEE)) {
          ...
          BDD$dist[i]<-sqrt((BDD$X[i] - as.numeric(BDD_score$X))^2+(BDD$Y[i]-as.numeric(BDD_score$Y))^2)
        ...
        for (i in 1:length(BDD$dist)) {
          ...
          if (BDD$dist[i]>800000) {
            ...
            BDD$score[i]<-as.numeric(BDD$score[i]) + 0.14
          } else if (BDD$dist[i]>400000) {
            BDD$score[i]<-as.numeric(BDD$score[i]) + 0.28
          } else if (BDD$dist[i]>200000) {
            BDD$score[i]<-as.numeric(BDD$score[i]) + 0.6
          } else if (BDD$dist[i]>80000) {
            BDD$score[i]<-as.numeric(BDD$score[i]) + 0.8
        ...
        BDD <- BDD[order(BDD$score,decreasing = TRUE),]
        
        output$BPs <- renderUI({
          
          lapply(1:length(BDD$`N°`), function(i) {
            tags$a(id=paste0("BP_",i),href=BDD$BP_link_plateforme[i],onclick="",tags$div(class="listBP_elem",tags$img(class="listBP_img",src=paste0("img/",BDD$`@IMG`[i])),tags$span(class="listBP_titre",paste0(BDD$INTITULE[i]," - ",BDD$TER[i]," (",BDD$ID[i],")")),tags$span(class="listBP_description",BDD$DESCRIPTION1[i])))
          })
  ...
  observeEvent(input$sidebarID,{
   ...
   BP_id<-aa[["url_search"]]
  BP_id <- substring(text = BP_id,first = 22,last = nchar(BP_id))
    
    
    if (nchar(BP_id)>0) {
      
      
      print(as.numeric(BP_id))
      ...
      output$BP_page<-renderUI({
        ...
        tags$div(class="BP_wrapper",
                 ...
                 tags$div(class="BP_content",
                          tags$div(class="BP_imgdownload",tags$img(class="BP_img",src=paste0("img/",BDD_BP$`@IMG`)), HTML(paste0('<a href="pdf/Bonnes Pratiques_Partie',BDD_BP$`N°`,'.pdf" download="',BDD_BP$INTITULE,'">Télécharger</a>'))),
                          ...
                          tags$div(class="BP_corr",tags$span())
                 )
        )
