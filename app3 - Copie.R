...
body<-dashboardBody(tags$head(HTML("
  ...
  shinybusy::busy_start_up(
    ...
    text = "Chargement...",
    mode = "timeout",background = "#00753B",timeout = 3000,color = "white"
  ),
  ...
  tags$div(id="decouvrir_wrapper",class="flex-column",
           ...
           tags$div(id="filtres",
                    ...
                    shinyWidgets::pickerInput(inputId = "Habitants",multiple=TRUE,selected = NULL,label=NULL,
                                              choices=c("Moins de 500 habitants","Entre 500 et 1 000 habitants","Entre 1 000 et 2 500 habitants","Entre 2 500 et 10 000 habitants","Entre 10 000 et 50 000 habitants","Entre 50 000 et 80 000 habitants","Entre 80 000 et 100 000 habitants","Plus de 100 000 habitants"),width = "208px",
                    ...
                    shinyWidgets::pickerInput(inputId = "Pays",selected=NULL,multiple = TRUE,label=NULL,choices=list("Département"=dep$CODE,"Pays"=c("Belgique","Espagne","Grèce","Irlande","Italie","Royaume-Uni","Slovénie")),width = "208px",options = pickerOptions(placeholder="Département/Pays", liveSearch = TRUE,`none-selected-text` = "Département/Pays",`actions-box` = TRUE,
                                                                                                                                                                 `deselect-all-text` = "Tout déselectionner",
                                                                                                                                                                 `select-all-text` = "Tout sélectionner")),
                    textInput(inputId = "Motscles",label=NULL,width = "208px", placeholder="Mots-clés"),
                    pickerInput(inputId = "BP_nb",label = "Afficher :",choices = c("20"=20,"50"=50,"100"=100,"200"=200,"500"=500,"Tout"=2000),selected = 20)),
           
           
           tags$div(class="green_div_column",
                    tags$div(id="BP",uiOutput("BPs"),uiOutput(outputId = "plusUI")))
  )
),
...
server <- function(input, output, session) {
  ...
  observeEvent(c(input$Motscles,input$Habitants,input$Thematiques,input$Pays,input$ok,input$BP_nb),{
    ...
    output$plusUI <- renderUI({
 
      
      if (input$BP_nb==2000) {
        
      } else if (as.numeric(input$BP_nb)>length(BDD$INTITULE)) {
    
      } else {
        tags$div(id="plusdiv",class="l",actionButton(inputId = "plus",label = "Suivant"))
      }
  ...
  observeEvent(input$plus,{
    shinyWidgets::updatePickerInput(inputId = "BP_nb",session = session,
    selected = if (input$BP_nb==20) {
      50
    } else if (input$BP_nb==50) {
      100
    } else if (input$BP_nb==100) {
      200
    } else if (input$BP_nb==200) {
      500
    } else {
      2000
    }
